...
function resolveNativeHelperPath() {
  const candidates = [
    path.join(process.cwd(), 'resources', 'native', 'AFAAudioHelper'),
    path.join(process.cwd(), 'native', 'macos', '.build', 'release', 'AFAAudioHelper'),
    process.resourcesPath ? path.join(process.resourcesPath, 'native', 'AFAAudioHelper') : null,
    process.resourcesPath ? path.join(process.resourcesPath, 'resources', 'native', 'AFAAudioHelper') : null
  ].filter(Boolean);
...
class AudioRouting {
  constructor() {
    this.nativeHelperPath = resolveNativeHelperPath();
  }

  async runNativeHelper(args) {
    if (!this.nativeHelperPath) {
      return null;
    }

    const result = await runCommand(this.nativeHelperPath, args);

    if (result.code !== 0) {
      throw new Error(result.stderr.trim() || result.stdout.trim() || 'Native audio helper failed.');
    }
  ...
  async listOutputDevices() {
    if (this.nativeHelperPath) {
      try {
        const raw = await this.runNativeHelper(['list-output-devices', '--json']);
        const parsed = JSON.parse(raw);
        const devices = Array.isArray(parsed) ? parsed.map(normalizeDeviceName).filter(Boolean) : [];

        if (devices.length > 0) {
          return devices;
        }
      } catch (error) {
        // Fall back to SwitchAudioSource if the helper fails.
      }
    }
  ...
  async getCurrentOutputDevice() {
    if (this.nativeHelperPath) {
      try {
        const raw = await this.runNativeHelper(['current-output', '--json']);
        const parsed = JSON.parse(raw);
        return normalizeDeviceName(parsed);
      } catch (error) {
        // Fall back to SwitchAudioSource if the helper fails.
      }
    }
  ...
  async setOutputDevice(name) {
    ...
    if (this.nativeHelperPath) {
      try {
        const raw = await this.runNativeHelper(['set-output', resolvedName, '--json']);
        const parsed = JSON.parse(raw);
        return normalizeDeviceName(parsed) || resolvedName;
      } catch (error) {
        // Fall back to SwitchAudioSource if the helper fails.
      }
    }
