...
class AfaController {
  ...
  getState() {
    return {
      ...
      lastSafeInput: this.config.lastSafeInput,
  ...
  async rememberCurrentSafeInput() {
    if (typeof this.audioRouting.getCurrentInputDevice !== 'function') {
      return;
    }

    try {
      const currentInput = await this.audioRouting.getCurrentInputDevice();
      const normalized = String(currentInput || '').trim();

      if (normalized && !this.isVirtualDeviceName(normalized)) {
        this.config.lastSafeInput = normalized;
      }
    } catch (error) {
      // Ignore read failures; recovery will fall back later.
    }
  ...
  async initialize(options = {}) {
    ...
    await this.rememberCurrentSafeInput();
  ...
  async shutdown() {
    ...
    if (typeof this.audioRouting.setInputDevice === 'function' && this.config.lastSafeInput) {
      try {
        const currentInput = await this.audioRouting.getCurrentInputDevice();

        if (currentInput && this.isVirtualDeviceName(currentInput)) {
          await this.audioRouting.setInputDevice(this.config.lastSafeInput);
        }
      } catch (error) {
        // Best-effort restore only.
      }
  ...
  async setOutput(value) {
    ...
    if (this.isVirtualDeviceName(target)) {
      ...
      try {
        ...
        if (this.platform === 'win32' && typeof this.audioRouting.setInputDevice === 'function') {
          await this.audioRouting.setInputDevice(this.listenerDevice);
        }
        this.config.output = resolvedOutput;
        this.config.speechOutput = resolvedOutput;
        this.config.routingMode = 'device';
        this.config.startupWarning = `Virtual mic ready on "${resolvedOutput}" with microphone "${this.listenerDevice}".`;
        return this.getState();
      } catch (_error) {
        this.config.output = target;
        this.config.speechOutput = null;
