...
class AfaController {
  ...
  async setOutput(value) {
    ...
    if (this.isVirtualDeviceName(target)) {
      try {
        ...
        // Fall back to output switching if direct speech routing cannot see the device.
      }
    }
  ...
  async getAudioSafety() {
    ...
    return {
      ...
      directVirtualRouting: this.config.routingMode === 'direct-say' && this.isVirtualDeviceName(this.config.output)
    };
  }
